...
/*
   ...
   You must NOT modify this file except to complete the TODO sections.
*/
...
int main() {
    ...
    // ============================
    // TODO 1: Create the root node
    // ============================
    adventureTree.createRoot(rawNodes[0].id, rawNodes[0].text);

    // ==============================
    // TODO 2: Add all remaining nodes
    // ==============================
    for (int i = 0; i < rawNodes.size(); i++)
    {
        StoryNodeRaw &parent = rawNodes[i];

        for (int j = 0; j < parent.children.size(); j++)    // Loop through each child ID under this parent.
        {
            string childID = parent.children[j];        // Get the child ID

            string childText = "";

            // Loop through rawnodes
            for (int k = 0; k < rawNodes.size(); k++)
            {
                // Check for a match
                if (rawNodes[k].id == childID)
                {
                    childText = rawNodes[k].text;
            ...
            // If childText is not empty, it means the child node was found.
            if (!childText.empty())
            {
                adventureTree.addNode(parent.id, childID, childText);
            }
        }
    }
